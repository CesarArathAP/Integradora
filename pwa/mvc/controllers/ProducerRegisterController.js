// mvc/controllers/ProducerRegisterController.js
const fs = require("fs");
...
const PRODUCER_REGISTER_VIEW_PATH = path.join(VIEWS_DIR, "producer_register.html"); // Nueva vista

class ProducerRegisterController {
    /**
     * Renderiza la vista estática del formulario de Registro de Nuevo Productor.
    ...
    static renderVista(req, res) {
        try {
            ...
            let content = fs.readFileSync(PRODUCER_REGISTER_VIEW_PATH, "utf8");
            ...
            console.error("Error al renderizar la vista de Registro de Productor (Estática):", err.message);
            res.status(500).send("Error interno al cargar la vista de registro de productor.");
        }
    }
}

// ⭐️ EXPORTACIÓN CORRECTA
module.exports = { ProducerRegisterController };